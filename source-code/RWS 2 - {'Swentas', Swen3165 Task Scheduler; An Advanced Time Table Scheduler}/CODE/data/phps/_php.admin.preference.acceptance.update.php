		<?php
			session_start(); 
				...
				$slotId = $_GET [ 'PREFERENCE_ACCEPTANCE_SLOT_ID' ];
				$instructorId = $_GET['PREFERENCE_ACCEPTANCE_INSTRUCTOR_ID'];
				$venue = $_GET [ 'PREFERENCE_ACCEPTANCE_VENUE' ];
				$courseCode = $_GET [ 'PREFERENCE_ACCEPTANCE_COURSE_CODE' ];
				$activity = $_GET [ 'PREFERENCE_ACCEPTANCE_ACTIVITY' ];
				$slotColour = $_GET [ 'PREFERENCE_ACCEPTANCE_SLOT_COLOUR' ];
				
				//manipulate database based on new user details 
				$connection -> query ("update Preferences set AcceptanceStatus='true' where SlotId='$slotId' and InstructorId='$instructorId' and Venue='$venue' and CourseCode='$courseCode' and Activity='$activity' and SlotColour='$slotColour'");
